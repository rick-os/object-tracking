...
def drawBox(img, bbox):
    ...
    cv2.putText(img, "Rastreando", (75, 90),
...
def goal_track(img, bbox):
    ...
    center_point = x+int(w/2)
    center_point2 = y+int(h/2)

    cv2.circle(img, (center_point, center_point2), 2, (0, 0, 255), 5)

    cv2.circle(img, (int(p1), int(p2)), 2, (0, 255, 0), 3)
    dist = math.sqrt(((center_point - p1)**2)+(center_point2 - p2)**2)
    print(dist)
    if (dist <= 20):
        cv2.putText(img, "Cesta", (300, 90),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
    xs.append(center_point)
    ys.append(center_point2)
